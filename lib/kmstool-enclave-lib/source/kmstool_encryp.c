...
static int encrypt_from_kms(
    ...
    const struct aws_string *plaintext_b64_str,
    struct aws_byte_buf *ciphertext_b64) {
    ssize_t rc = AWS_OP_ERR;

    struct aws_byte_buf plaintext;
    rc = decode_b64(ctx, plaintext_b64_str, &plaintext);
    if (rc != AWS_OP_SUCCESS) {
        fprintf(stderr, "Could not decode plaintext\n");
        return rc;
    }
    ...
    rc = aws_kms_encrypt_blocking(
        ctx->kms_client, ctx->key_id, &plaintext, &ciphertext);
    ...
    rc = encode_b64(ctx, &ciphertext, ciphertext_b64);
